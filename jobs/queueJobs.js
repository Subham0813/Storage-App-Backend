  ...
  async (job) => {
    ...
    switch (job.name) {
      ...
      case "trash-collector":
        ...
        await Promise.all(
          Array.from(deletedByUser.entries()).map(([userId, entry]) => {
            const parts = [];
            if (entry.files)
              parts.push(`${entry.files} file${entry.files !== 1 ? "s" : ""}`);
            if (entry.dirs)
              parts.push(`${entry.dirs} folder${entry.dirs !== 1 ? "s" : ""}`);
      ...
      case "halted-subscription-reaper":
        ...
        // Find subscriptions halted for more than 7 days
        const haltedCutoff = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        const haltedSubs = await Subscription.find({
          status: "halted",
          updatedAt: { $lte: haltedCutoff },
        }).lean();
        ...
        for (const sub of haltedSubs) {
          ...
          await User.findByIdAndUpdate(sub.user, {
            ...
            subscriptionExpiresAt: null,
            gracePeriodEndsAt: new Date(
              Date.now() + freePlan.gracePeriod * 24 * 60 * 60 * 1000,
            ),
          });
          await Subscription.findByIdAndUpdate(sub._id, {
            status: "completed",
        ...
        console.log(`Reaped ${haltedSubs.length} halted subscriptions.`);
        break;
...
export const startBullMQJobs = async () => {
  ...
  try {
    ...
    await backgroundQueue.add(
      "bandwidth-reset",
      {},
      { repeat: { pattern: "0 3 * * *" } }, // Daily at 3am (rolling per-user windows)
    );
    await backgroundQueue.add(
      "halted-subscription-reaper",
      {},
      { repeat: { pattern: "0 2 * * *" } }, // Daily at 4am
    );
    await backgroundQueue.add(
